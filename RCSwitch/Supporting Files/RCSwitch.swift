...
public protocol  LabelSwitchDelegate : class {
    func switchChangToState(sender: RCSwitch) -> Void
}
...
private class LabelSwitchPart {
    ...
    let back: RCSwitchBackView = {
        let view = RCSwitchBackView()
        return view
    }()
    ...
    func setConfig(_ config: RCSwitchConfig) {
    ...
    func setState(_ state: RCSwitchPartState) {
        mask.frame = state.backMaskFrame
        back.frame = state.backMaskFrame
    }
}

@IBDesignable public class RCSwitch: UIView {
    
    private lazy var circleView: UIView = {
        let view = UIView()
        view.backgroundColor = .orange //.white
        ...
        thumbImageView.center = view.center
        thumbImageView.frame.size = CGSize(width: 40, height: 40)
    ...
    private var stateL = RCSwitchState()
    private var stateR = RCSwitchState()
    ...
    public init(center: CGPoint,
            leftConfig: RCSwitchConfig,
           rightConfig: RCSwitchConfig,
    ...
    private func setConfig(left: RCSwitchConfig, right: RCSwitchConfig) {
        switchConfigL = left
        switchConfigR = right
    }
    ...
    private func updateUIState(_ state: RCSwitchState) {
    ...
    required public init?(coder aDecoder: NSCoder) {
        self.switchConfigL = RCSwitchConfig.defaultLeft
        self.switchConfigR = RCSwitchConfig.defaultRight
